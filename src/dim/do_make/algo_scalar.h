...
#ifndef _DO_MAKE_ALGO_SCALAR_H
#define _DO_MAKE_ALGO_SCALAR_H
...
namespace dim
{
	 ...
	 */
	template <class EOT>
	eoAlgo<EOT> & algo_scalar(eoParser& _parser, eoState& _state, eoEvalFunc<EOT>& _eval, eoContinue<EOT>& _continue, eoGenOp<EOT>& _op, eoDistance<EOT> * _dist = NULL)
	{
...
#include <ga/eoBit.h>
	...
	// the algo
	eoAlgo<eoBit<double> >&  algo_scalar(eoParser& _parser, eoState& _state, eoEvalFunc<eoBit<double> >& _eval, eoContinue<eoBit<double> >& _continue, eoGenOp<eoBit<double> >& _op, eoDistance<eoBit<double> >* _dist)
	{
	    return algo_scalar(_parser, _state, _eval, _continue, _op, _dist);
	}

	eoAlgo<eoBit<eoMinimizingFitness> >&  algo_scalar(eoParser& _parser, eoState& _state, eoEvalFunc<eoBit<eoMinimizingFitness> >& _eval, eoContinue<eoBit<eoMinimizingFitness> >& _continue, eoGenOp<eoBit<eoMinimizingFitness> >& _op, eoDistance<eoBit<eoMinimizingFitness> >* _dist)
	{
	    return algo_scalar(_parser, _state, _eval, _continue, _op, _dist);
	}
